...
export async function getAllCountries() {
  ...
  try {
    // Split fields into chunks of 9 (plus cca3 = 10 max) to avoid 400 Bad Request
    const chunkSize = 9;
    const chunks = [];
    for (let i = 0; i < fieldList.length; i += chunkSize) {
      const chunk = fieldList.slice(i, i + chunkSize);
      // Ensure cca3 is requested for merging, unless it's already there
      if (!chunk.includes("cca3")) {
          chunk.push("cca3");
      }
      chunks.push(chunk);
    }
    ...
    const promises = chunks.map(async (chunk) => {
      ...
      if (!res.ok) throw new Error(`Failed to fetch countries chunk: ${res.status} ${res.statusText}`);
      return res.json();
    });

    const results = await Promise.all(promises);

    // Merge chunks by cca3
    const mergedMap = {};
    results.forEach((chunkData) => {
      chunkData.forEach((c) => {
        if (!c.cca3) return; // Should not happen if API works
        if (!mergedMap[c.cca3]) {
            mergedMap[c.cca3] = c;
        } else {
            // Merge properties
            Object.assign(mergedMap[c.cca3], c);
        }
      });
    });

    const data = Object.values(mergedMap);
