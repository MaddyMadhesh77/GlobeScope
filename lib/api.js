...
export async function getAllCountries() {
  ...
  const chunkSize = 9; // Limit is 10 fields per request
  const chunks = [];
  for (let i = 0; i < fieldList.length; i += chunkSize) {
    chunks.push(fieldList.slice(i, i + chunkSize));
  }

  const responses = await Promise.all(
    chunks.map((chunk) => {
      const fields = [...chunk, "cca3"].join(",");
      return fetch(`https://restcountries.com/v3.1/all?fields=${fields}`, {
        cache: "no-store",
        headers: {
  ...
  results.flat().forEach((country) => {
    if (mergedData.has(country.cca3)) {
      Object.assign(mergedData.get(country.cca3), country);
    } else {
      mergedData.set(country.cca3, country);
    }
  });
